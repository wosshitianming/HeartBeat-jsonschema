...
public class AuthController {
     ...
     */
    @PostMapping("/login")
    public Result<DynamicRecordResponse> login(@RequestBody LoginRequest request) {
        // 转换登录请求为平台服务字段映射。
        Map<String, Object> payload = request.toMap();
        // 调用平台服务完成认证并签发令牌。
        Map<String, Object> loginResult = adminPlatformService.login(payload);
        // 将动态登录结果转换为统一响应对象。
        DynamicRecordResponse response = DynamicRecordResponse.from(loginResult);
        // 返回登录成功响应。
    ...
    public Result<DynamicRecordResponse> me(
        ...
        // 查询当前用户聚合信息。
        Map<String, Object> currentUser = adminPlatformService.currentUser();
        // 将动态用户上下文转换为统一响应对象。
    ...
    public Result<DynamicRecordResponse> appearancePreference(
            ...
            @RequestHeader(value = "Authorization", required = false) String authorization) {
        // 读取当前用户外观偏好。
        Map<String, Object> preference = adminPlatformService.appearancePreference();
        // 将动态偏好配置转换为统一响应对象。
    ...
    public Result<DynamicRecordResponse> updateAppearancePreference(
        ...
        Map<String, Object> payload = request.toMap();
        // 持久化当前用户外观偏好。
        Map<String, Object> saved = adminPlatformService.updateAppearancePreference(payload);
        // 将动态偏好配置转换为统一响应对象。
     ...
     */
    @PostMapping("/refresh")
    public Result<DynamicRecordResponse> refresh(@RequestBody RefreshTokenRequest request) {
        // 读取刷新令牌。
        String refreshToken = request.getRefreshToken();
        // 校验服务端会话并轮换刷新令牌。
        Map<String, Object> tokens = authenticationSessionService.refresh(refreshToken);
        // 将动态令牌结果转换为统一响应对象。
        DynamicRecordResponse response = DynamicRecordResponse.from(tokens);
        // 返回新令牌响应。
        return Result.success(response);
    }
    ...
    public Result<List<SocialProviderResponse>> socialProviders() {
        // 查询登录页精简渠道列表。
        List<Map<String, Object>> providers = socialLoginService.listProvidersForLogin();
        // 转换为第三方登录渠道响应对象列表。
        List<SocialProviderResponse> response = SocialProviderResponse.fromList(providers);
        // 返回第三方登录渠道列表。
        return Result.success(response);
    }
    ...
    public Result<DynamicRecordResponse> socialAuthorize(
        ...
        // 生成第三方授权地址与状态值。
        Map<String, Object> authorize = socialLoginService.buildAuthorizeUrl(provider, redirect);
        // 将动态授权信息转换为统一响应对象。
    ...
    public Result<DynamicRecordResponse> socialCallbackGet(
        ...
        // 处理第三方 OAuth 回调。
        Map<String, Object> callbackResult = socialLoginService.handleCallback(provider, code, state);
        // 将动态回调结果转换为统一响应对象。
    ...
    public Result<DynamicRecordResponse> socialBind(@RequestBody SocialBindRequest request) {
        ...
        // 校验本地账号并建立第三方绑定关系。
        Map<String, Object> bindResult = socialLoginService.bindExistingAccount(bindTicket, username, password);
        // 将动态绑定结果转换为统一响应对象。
